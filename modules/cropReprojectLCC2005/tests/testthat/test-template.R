...
#    `functionName` is a function's name in your module (e.g., `cropReprojectLCC2005Event1`).
# 2. Copy this file to the tests folder (i.e., `C:/Users/tmichele/Dropbox/Project SpaDES/SpaDES-modules/modules/cropReprojectLCC2005/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("cropReprojectLCC2005")
  path <- list(modulePath = "C:/Users/tmichele/Dropbox/Project SpaDES/SpaDES-modules/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    cropReprojectLCC2005 = list(.saveInitialTime = NA)
  )
  ...
  if (exists("cropReprojectLCC2005Event1", envir = .GlobalEnv)) {
    simOutput <- cropReprojectLCC2005Event1(mySim)
  } else {
  ...
  if (exists("cropReprojectLCC2005Event2", envir = .GlobalEnv)) {
    simOutput <- cropReprojectLCC2005Event2(mySim)
  } else {
